    ...
    if submitted:
        try:
            ...
            if explainer is None:
                st.warning("SHAP disabled. Place `train_data.csv` in project root.")
            else:
                ...
                try:
                    fp = shap.force_plot(bv, sv, x_df.iloc[0],
                                         feature_names=feature_names, matplotlib=False)
                    st_shap(fp, height=190)
                except Exception as e:
        ...
        try:
            ...
            if st.button("Explain", type="primary"):
                if explainer is None:
                    ...
                    exp_obj = shap.Explanation(values=sv, base_values=bv,
                    ...
                    try:
                        fp = shap.force_plot(bv, sv, X_b.iloc[idx],
                                             feature_names=feature_names, matplotlib=False)
                        st_shap(fp, height=190)
                    except Exception:
                        pass
...
with tab3:
    ...
    if hasattr(model, "coef_"):
        ...
        df_coef.index += 1
        st.dataframe(df_coef[["Feature", "Coefficient", "Odds Ratio"]],
        ...
        ax.barh(dfp["Feature"], dfp["Coefficient"], color=colors, alpha=0.9, edgecolor="white")
